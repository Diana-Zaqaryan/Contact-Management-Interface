...
function Contacts({
  ...
  const filteredContacts: Contact[] = data
    ? (data as Contact[])
        .sort((a: Contact, b: Contact) => parseInt(b.id) - parseInt(a.id))
    ...
    <>
      <div className={styles.search_wrapper}>
        ...
        <div className={styles.search_content}>
          ...
          <Link to="/createNew">
            <button onClick={resetSelectedContactId} className={styles.new_btn}>
              New
            </button>
          </Link>
        </div>
        ...
        {filteredContacts.map((contact: Contact) => {
          return (
            <Link key={contact.id} to={`contacts/${contact.id}`}>
              <li
                className={
                  selectedContactId === contact.id ? styles.active : styles.item
                }
